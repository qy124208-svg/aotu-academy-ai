  ...
  if(reqPath==='/api/batch' && req.method==='POST'){
    let body='';req.on('data',c=>body+=c);req.on('end',async ()=>{
      try{
        ...
        const sp=base+'\n\n为以下每个场景各写1段叙事(40字内)和2个选项(各12字内)。严格JSON格式：\n{"chat":{"n":"叙事","c":["选项1","选项2"]},"help":{...},...}\n共15个场景。用「」引号，用——停顿。';
        const up='场景列表：'+types.map(t=>{
          const lb={chat:'闲聊偶遇',help:'需要帮助',deep:'深度谈心',fun:'轻松搞笑',study:'一起学习',lunch:'午餐时间',sports:'体育活动',hallway:'走廊相遇',walk_home:'放学回家',rain:'雨天',sick:'身体不舒服',comfort:'安慰时刻',share:'分享东西',game:'一起玩游戏',homework:'写作业'};
          return lb[t]||t;
        }).join('、');
        const r=await fetch('https://api.deepseek.com/v1/chat/completions',{
          method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+AI_KEY},
